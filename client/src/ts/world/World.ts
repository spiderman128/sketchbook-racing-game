...
import { RACING_GAME_ID } from "../constant";
...
export class World {
  ...
  public UIcontrollers: any;
  public playerList: any[];
  public roomList: any[];
  public roomId: string;
  public playerId: string;
  public spawnNumber: number;
  public gameStatus: boolean = false; // true: started
	public racingPoints: Array<any>;
  public racingPointId: number;
  ...
  constructor(worldScenePath?: any) {
    ...
    // Get socket client
    this.socket = SocketClient.getInstance(this);
  ...
  public showMultiUI(isCreate: boolean): void {
    if (isCreate) {
      this.clearMultiUI();
      this.isAdmin = true;
      this.params.Player_Number = 1;
      this.UIcontrollers.playerNumber = this.scenarioGUIFolder.add(
        this.params,
        "Player_Number",
        1
      );
      this.UIcontrollers.startGame = this.scenarioGUIFolder.add(
        this.params,
        "Start_Game"
      );

      this.scenarioGUIFolder.updateDisplay();
    } else {
      this.clearMultiUI();
      this.isAdmin = false;
      this.UIcontrollers.roomSelect = this.scenarioGUIFolder.add(
        this.params,
        "Room_Select",
        []
      );
      this.UIcontrollers.joinRoom = this.scenarioGUIFolder.add(
        this.params,
        "Join_Room"
      );
  ...
  public clearMultiUI(): void {
    if (this.UIcontrollers.playerNumber)
      this.scenarioGUIFolder.remove(this.UIcontrollers.playerNumber);
    if (this.UIcontrollers.startGame)
      this.scenarioGUIFolder.remove(this.UIcontrollers.startGame);
    if (this.UIcontrollers.roomSelect)
      this.scenarioGUIFolder.remove(this.UIcontrollers.roomSelect);
    if (this.UIcontrollers.joinRoom)
      this.scenarioGUIFolder.remove(this.UIcontrollers.joinRoom);
    if (this.UIcontrollers.quitGame)
      this.scenarioGUIFolder.remove(this.UIcontrollers.quitGame);
  ...
  public startGame(): void {
    ...
    this.racingPointId = -1;
    this.launchScenario(RACING_GAME_ID);
    this.gameStatus = true;
    this.clearMultiUI();

    this.UIcontrollers.quitGame = this.scenarioGUIFolder.add(this.params, "Quit_Game");
    this.scenarioGUIFolder.updateDisplay();
  }
  ...
  public updatePhysics(timeStep: number): void {
    ...
    this.vehicles.forEach((vehicle) => {
      if (this.isOutOfBounds(vehicle.rayCastVehicle.chassisBody.position) && vehicle.playerId && vehicle.playerId === this.playerId) {
    ...
    if(this.gameStatus) {
      const vehicle = this.vehicles.find(item => item.playerId === this.playerId);
      if(vehicle && this.racingPointId !== this.racingPoints.length - 1) {
        $("#gamestatus").html("Status: " + (this.racingPointId + 1) + "/" + this.racingPoints.length);
        const curPosition = vehicle.rayCastVehicle.chassisBody.position;
        const nextPosition = this.racingPoints[this.racingPointId + 1];
        
        // Calculate the distance to next point
        const distance = Math.sqrt(Math.pow(nextPosition.x - curPosition.x, 2) + Math.pow(nextPosition.z - curPosition.z, 2));
        if(distance < 20) {
          this.racingPointId++;
          if(this.racingPointId === this.racingPoints.length - 1) {
            this.socket.sendArrive();
            $("#gamestatus").html("Status: Arrived");
          }
        }
        this.socket.sendGameData(vehicle.rayCastVehicle.chassisBody);
      }
    }
  }
  ...
  private generateHTML(): void {
    ...
    $(`	<div id="ui-container" style="display: none;">
        ...
        <div class="right-panel">
          <div id="gamestatus">Milestone: None</div>
          <div id="rank" style="margin-top:20px">WinnerList: None</div>
        </div>
			</div>
		`).appendTo("body");
  ...
  private createParamsGUI(scope: World): void {
    this.params = {
      ...
      Player_Number: 0,
      Join_Room: () => {
        if (this.params.Room_Select === "") {
          alert("Try again!");
        } else {
          this.socket.joinRoom(this.params.Room_Select);
        }
      },
      Start_Game: () => {
    ...
    // Scenario
    this.scenarioGUIFolder = gui.addFolder("MultiPlay");
